		...
		try {
			...
			if ( $comment_id ) {
				...
				if ( $comment ) {
					...
					if ( get_comment_meta( $comment_id, 'id_import_reviews_from_ali', true ) ||
					     get_comment_meta( $comment_id, 'id_import_reviews_from_amazon', true ) ||
					     get_comment_meta( $comment_id, 'id_import_reviews_from_temu', true ) ) {
					...
					if ( is_array( $reviews_images ) && !empty( $reviews_images ) ) {
						$images = array();
						...
						foreach ( $reviews_images as $key => $value ) {
							...
							if ( villatheme_is_url( $value ) ) {
								...
								$image_id = vi_wcpr_handle_sideload_unique( $value, $comment_id, $comment->comment_post_ID );
