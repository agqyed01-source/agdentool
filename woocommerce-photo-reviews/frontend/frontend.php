...
class VI_WOOCOMMERCE_PHOTO_REVIEWS_Frontend_Frontend {
    ...
    public static function send_schedule_email( $user_email, $customer_name, $products, $order_id, $time, $date_create, $date_complete ) {
        ...
        if ( count( $products ) ) {
            ...
            if ( self::$settings->get_params( 'multi_language' ) ) {
                ...
                if (!$language){
                    $language = $order->get_meta('trp_language');
                }
            }
            $subject        = $content = '';
            ...
            if ( $email_template && VI_WOOCOMMERCE_PHOTO_REVIEWS_DATA::is_email_template_customizer_active() ) {
                ...
                if ( $email_template_obj && $email_template_obj->post_type === 'viwec_template' ) {
                    ...
                    $use_template = true;
                    add_filter( 'viwec_disable_woocommerce_email_inline_style', '__return_false', 999 );
    ...
    public static function send_schedule_email1( $user_email, $customer_name, $products, $order, $date_create, $date_complete, $language, $token = '', $user_id = '' ) {
        ...
        foreach ( $products as $p ) {
            ...
            if ( $product ) {
                ...
                if ( $token ) {
                    $product_url = add_query_arg( 'wcpr_token', $token, $product_url );
                    $product_url1 = add_query_arg( 'wcpr_token', $token, $product_url1 );
                }else{
                    $arg_t = [
                    ...
                    $product_url = add_query_arg( $arg_t, $product_url );
                    $product_url1 = add_query_arg( $arg_t, $product_url1 );
                }
                $product_title = $product->get_title();
                do_action( 'viwcpr_reminder_before_get_product_html', $order, $products );
                $product_price = apply_filters( 'woocommerce_photo_reviews_reminder_product_price', $product->get_price_html(), $product, $order );
                ...
                <tr>
                    ...
                    <td>
                        ...
                        if ( $show_star_rating ) {
                            ...
                            $product_url1 = add_query_arg( $arg_t,  $product_url1 );
                            ?>
                            <p><a target="<?php echo esc_attr( $target_link ) ?>" style="display: flex; margin-bottom: 16px;" href="<?php echo esc_url( $product_url1 ) ?>"><img src="<?php echo esc_url( VI_WOOCOMMERCE_PHOTO_REVIEWS_IMAGES . 'star.png' ) ?>" alt="star" style="width: 20px;height: 20px;margin-right: 2px;"><img src="<?php echo esc_url( VI_WOOCOMMERCE_PHOTO_REVIEWS_IMAGES . 'star.png' ) ?>" alt="star" style="width: 20px;height: 20px;margin-right: 2px;"><img src="<?php echo esc_url( VI_WOOCOMMERCE_PHOTO_REVIEWS_IMAGES . 'star.png' ) ?>" alt="star" style="width: 20px;height: 20px;margin-right: 2px;"><img src="<?php echo esc_url( VI_WOOCOMMERCE_PHOTO_REVIEWS_IMAGES . 'star.png' ) ?>" alt="star" style="width: 20px;height: 20px;margin-right: 2px;"><img src="<?php echo esc_url( VI_WOOCOMMERCE_PHOTO_REVIEWS_IMAGES . 'star.png' ) ?>" alt="star" style="width: 20px;height: 20px;"></a></p>
                            <?php
                        }
                        ?>
                        <p><a target="<?php echo esc_attr( $target_link ) ?>"
            ...
            if ( $order ) {
                ...
                if ( count( $products ) ) {
                    ...
                    if ( $user_id ) {
                        ...
                        $token                  = '';
                        if (!is_array($user_reviewed_products)){
                        ...
                        if ( !empty( $not_reviewed_products ) ) {
                            if ( $auto_login ) {
        ...
        if ( self::$settings->get_params( 'multi_language' ) ) {
            ...
            if ( $current_language ) {
                ...
                if ( is_plugin_active( 'sitepress-multilingual-cms/sitepress.php' ) ) {
                    ...
                    $default_lang = $trp_settings['default-language']??get_locale();
                }
                if ($default_lang && $current_language !== $default_lang ) {
    ...
    public static function upload_file( $comment_id, $post_id ) {
            ...
            foreach ( $files['name'] as $key => $value ) {
                ...
                if ( is_array( $value ) ) {
                    ...
                    foreach ( $value as $sub_key => $sub_value ) {
                        ...
                        $file                   = array(
                                'name'     => apply_filters( 'woocommerce_photo_reviews_image_file_name', $sub_value, $comment_id, $post_id, false ),
                                'type'     => $files['type'][ $key ][ $sub_key ],
                                'tmp_name' => $files['tmp_name'][ $key ][ $sub_key ],
                                'error'    => $files['error'][ $key ][ $sub_key ],
                                'size'     => $files['size'][ $key ][ $sub_key ]
                        );
                    ...
                    $file                   = array(
                            'name'     => apply_filters( 'woocommerce_photo_reviews_image_file_name', $value, $comment_id, $post_id, false ),
                            'type'     => $files['type'][ $key ],
        ...
        return ( $error && empty( $img_id ) ) ? '' : $img_id;
    }
    ...
    public function login() {
        ...
        $token = sanitize_text_field( wp_unslash($_GET['wcpr_token']) );
        if ( ! is_user_logged_in() && self::$settings->get_params( 'followup_email', 'auto_login' ) ) {
            ...
            if ( $user_id && ( $user = get_user_by( 'id', $user_id ) ) ) {
                ...
                if ( $wcpr_token === $token && empty( array_intersect( $auto_login_exclude, $user->roles ) ) ) {
                    ...
                    wp_safe_redirect( remove_query_arg( 'wcpr_token' )  );
                    exit();
                } else {
    ...
    public static function get_language() {
        ...
        if ( is_plugin_active( 'sitepress-multilingual-cms/sitepress.php' ) ) {
            ...
            if (!empty($_REQUEST['trp-form-language'])){
                $current_language = wc_clean(wp_unslash($_REQUEST['trp-form-language']));
        ...
        if ( $default_lang && $current_language && $current_language !== $default_lang ) {
    ...
    public function add_comment_field( $comment_form ) {
        ...
        if ( self::$settings->get_params( 'allow_empty_comment' ) ) {
            ...
            preg_match( '/<textarea id="comment".*<\/textarea>/', $comment_field, $match );
            if ( ! empty( $match[0] ) ) {
                $tmp           = preg_replace( '/ (required|required[\=][\"][\"]|required[\=][\'][\'])/i', '', $match[0] );
                $comment_field = preg_replace( '/<textarea id="comment".*<\/textarea>/', $tmp, $comment_field );
            }
        }
    ...
    public function comment_author( $author, $comment_id ) {
        ...
        if ( wp_doing_ajax() && ! isset( $_REQUEST['action'] ) && ( $_REQUEST['action'] == 'wcpr_ajax_load_more_reviews' ) ) {
            return $author;
        }
        $comment = get_comment( $comment_id );
        if ( ! $comment ) {
    ...
    public function ajax_load_more_reviews() {
        ...
        $response['$count_reviews']        = $count_reviews ?? 'd';
        $response['default_comments_page'] = get_option( 'default_comments_page' );
        if ( $frontend_style == 2 && $cpage === 1 && get_option( 'default_comments_page' ) === 'oldest' ) {
            if ( isset( $count_reviews, $response['max_page'] ) ) {
                ...
                $count_reviews      = self::get_comments( apply_filters( 'woocommerce_photo_reviews_shortcode_comment_count_args', $comment_count_args ) );
                $max_page           = ceil( $count_reviews / $comments_per_page );
            }
            $comments_per_page              = $count_reviews - ( ( $max_page - 1 ) * $comments_per_page );
            $comment_args['number']         = $comments_per_page;
            $response['$comments_per_page'] = $comments_per_page;
        }
        ...
        switch ( $frontend_style ) {
            ...
            case '6':
                do_action( 'viwcpr_get_template_list_layout_2_html', array(
                        ...
                        'parent_tag_html'   => $r['style'] ?? '',
                        'cols'              => self::$settings->get_params( 'photo', 'col_num' ),
                        'cols_mobile'       => self::$settings->get_params( 'photo', 'col_num_mobile' ),
                        'masonry_popup'     => self::$settings->get_params( 'photo', 'masonry_popup' ),
                        'enable_box_shadow' => self::$settings->get_params( 'photo', 'enable_box_shadow' ),
                        'loadmore_button'   => self::$settings->get_params( 'pagination_ajax' ) && self::$settings->get_params( 'loadmore_button' ) ? 1 : '',
                        'show_product'      => 'off',
    ...
    public function viwcpr_get_overall_rating_html( $arg ) {
        ...
        $style = $this->frontend_style;
        switch ( $arg['average_rating_style'] ?? '' ) {
            case 1:
                $style = 1;
                break;
            case 2:
