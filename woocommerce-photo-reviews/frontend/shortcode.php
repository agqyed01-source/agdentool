	...
	public function overall_rating_html( $atts ) {
		...
		$enqueue_args = [
			'style'  => [
				...
				'wcpr-shortcode-reviews-list-style',
				'wcpr-swipebox-css',
		...
		$this->enqueue_scripts($enqueue_args);
			...
			if ( $arr['is_elementor'] !== 'yes' ) {
					...
					if ( $arr['is_slide'] !== 'on' ) {
						$custom_css .= VI_WOOCOMMERCE_PHOTO_REVIEWS_Frontend_Frontend::add_inline_style(
							array(
								...
								$shortcode_id_css . '.shortcode-wcpr-grid.wcpr-grid-loadmore',
							),
							array( 'column-count', 'grid-template-columns', 'display' ),
							array( $arr['cols_mobile'], 'repeat(' . $arr['cols_mobile'] . ', 1fr)', 'grid !important' ),
							array( '!important', '!important' )
						);
						$custom_css .= self::$frontend::add_inline_style(
							array(
								$shortcode_id_css . '.shortcode-wcpr-grid .shortcode-wcpr-grid-style-item',
								$shortcode_id_css . '.shortcode-wcpr-grid .shortcode-wcpr-grid-layout-2-style-item',
							),
							array( 'width' ),
							array( '100%' ),
							array( '!important' )
						...
						if ( $arr['cols'] ) {
							$custom_css .= $shortcode_id_css . '.shortcode-wcpr-grid{column-count: 1 !important;}';
						}
					}
					$custom_css .= '}';
				}
				if ( 'on' === $arr['filter'] ) {
					...
					if ( $arr['area_border_color'] ) {
						$custom_css .= "border-color:" . $arr['area_border_color'] . ' !important;';
					}
				...
				switch ( $arr['pagination_position'] ?? '' ) {
					case 'left':
						$custom_css .= "{$shortcode_id_css}.shortcode-wcpr-pagination{justify-content: flex-start;}";
						break;
					case 'right':
						$custom_css .= "{$shortcode_id_css}.shortcode-wcpr-pagination{justify-content: flex-end;}";
						break;
				}
...
//				}
		...
		$loadmore_button = 'on' === $arr['pagination'] && $max_num_pages > 1 && $arr['loadmore_button'] === 'on' && $arr['pagination_ajax'] === 'on' && $arr['is_slide'] !== 'on';
		ob_start();
		if ( ! $this->is_ajax ) {
			$data_arr = $arr;
		...
		if ( ( $arr['overall_rating'] === 'on' || $arr['rating_count'] === 'on' ) && ( $count_reviews || $arr['hide_rating_count_if_empty'] !== 'on' ) ) {
			...
			do_action( 'viwcpr_get_overall_rating_html', array(
				...
				'average_rating_style'  => $arr['average_rating_style'],
		...
		if ( is_array( $my_comments ) && ! empty( $my_comments ) ) {
			$pagination_html = '';
			...
			printf( '%s ', $loadmore_button ? '' : ( empty( $arr['pagination_end'] ) ? $pagination_html : '' ) );
	...
	public function enqueue_scripts($enqueue_args = []) {
	...
	public function wp_enqueue_scripts_elementor() {
		...
		wp_enqueue_style( 'wcpr-shortcode-reviews-list-style', VI_WOOCOMMERCE_PHOTO_REVIEWS_CSS . 'shortcode-list-style.' . $suffix . 'css', array(), VI_WOOCOMMERCE_PHOTO_REVIEWS_VERSION );
	...
	public function wp_enqueue_scripts() {
		...
		if ( ! wp_style_is( 'wcpr-shortcode-all-reviews-style', 'registered' ) ) {
			...
			wp_register_style( 'wcpr-shortcode-reviews-list-style', VI_WOOCOMMERCE_PHOTO_REVIEWS_CSS . 'shortcode-list-style.' . $suffix . 'css', array(), VI_WOOCOMMERCE_PHOTO_REVIEWS_VERSION );
		}
