    ...
    if ($rating_count_enable === 'on') {
        ...
        <div class="<?php echo esc_attr($prefix_class); ?>wcpr-stars-count <?php echo esc_attr($prefix_class); ?>list-style">
            ...
            for ($i=5; $i > 0; $i--){
                $tmp_star_count = 0;
                if ($count_reviews) {
                    $tmp_star_count = isset($star_counts[$i]) ? $star_counts[$i] : ($product_id ? VI_WOOCOMMERCE_PHOTO_REVIEWS_Frontend_Frontend::stars_count($i, $product_id) : 0);
                    $max_star_count = max($max_star_count, $tmp_star_count);
                }
                $star_count[$i]=$tmp_star_count;
            }
            for ($i = 5; $i > 0; $i--) {
                $tmp_star_count = $star_count[$i];
                $rate = 0;
                if ($max_star_count){
                    $rate = (100 * ($tmp_star_count / $max_star_count));
                }
                if (!$rate){
                    $rate =1;
                }
...
//                }
                ...
                <div class="<?php echo esc_attr($prefix_class); ?>wcpr-row">
                    ...
                    <div class="<?php echo esc_attr( $prefix_class ); ?>wcpr-col-star"><?php echo wc_get_rating_html( 1 );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
                    <div class="<?php echo esc_attr($prefix_class); ?>wcpr-col-process">
                        <div class="rate-percent-bg">
                            ...
                            <div class="rate--count"><?php echo esc_html( $tmp_star_count) ?></div>
                        </div>
                    </div>
                </div>
                <?php
