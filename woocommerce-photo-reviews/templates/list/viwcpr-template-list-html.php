                ...
                <div class="<?php echo esc_attr( $prefix_class ); ?>review-wrap-content-container">
                    ...
                    <div class="<?php echo esc_attr( $prefix_class ); ?>review-content-image">
                        <div class="<?php echo esc_attr( $prefix_class ); ?>reviews-images-container">
                                ...
                                if ( count( $img_post_ids ) > 1 ) {
                                    foreach ( $img_post_ids as $img_post_ids_k => $img_post_id ) {
                                        if ( ! villatheme_is_url( $img_post_id ) ) {
                                            ...
                                            if ( $is_video ) {
                                                printf( '<div class="%sreviews-images-wrap"><a data-image_index="%s" data-image_src="%s" data-image_caption="%s" href="%s"><video class="%sreviews-images reviews-videos" data-src="%s"  >%s</video></a></div>',
                                                ...
                                                printf( '<div class="%sreviews-images-wrap"><a data-image_index="%s" data-image_src="%s" data-image_caption="%s" href="%s"><img class="%sreviews-images" loading="lazy" data-src="%s" alt="%s"></a></div>',
                                            ...
                                            if ( ! in_array( 'image/' . strtolower( $file_type ), $settings->get_params( 'upload_allow_images' ) ) ) {
                                                if ( strpos( $img_post_id, '.mp4' ) || strpos( $img_post_id, '.webm' ) || strpos( $img_post_id, '.mov' ) ) {
                                                    printf( '<div class="%sreviews-images-wrap"><a data-image_index="%s" href="%s"><video class="%sreviews-images reviews-videos" data-src="%s" >%s</video></a></div>',
                                                    ...
                                                    printf( '<div class="%sreviews-images-wrap"><a data-image_index="%s" href="%s"><video class="%sreviews-images reviews-iframe" data-src="%s" >%s</video></a></div>',
                                                            esc_attr( $prefix_class ), esc_attr( $img_post_ids_k ), esc_attr( $img_post_id ),
                                                            esc_attr( $prefix_class ), esc_url( $img_post_id ), esc_attr( $product_title )
                                                    );
                                                }
                            ...
                            echo wp_kses_post('<div class="'.$prefix_class.'reviews-images-wrap-right">');
                            if ( ! villatheme_is_url( $first_ele ) ) {
                                ...
                                if ( $caption_enable ) {
                                    printf( '<div class="%swcpr-review-image-container">',  esc_attr( $prefix_class ) );
                                    ...
                                    if ( $is_video ) {
                                        printf( '<video class="%sreviews-images reviews-videos" data-original_src="%s" src="%s" width="%s" height="%s" controls %s>%s</video>',
                                                esc_attr( $prefix_class ), esc_attr( $data_original_src ),
                                                esc_url( apply_filters( 'woocommerce_photo_reviews_grid_thumbnail_main', $src, $first_ele ) ),
                                                esc_attr( $img_width ), esc_attr( $img_height ), esc_attr( $auto_play_video ? 'autoplay muted' : '' ), esc_attr( $image_alt )
                                        );
                                    } else {
                                        printf( '<img class="%sreviews-images" loading="lazy" data-original_src="%s" src="%s" alt="%s" title="%s" width="%s" height="%s">',
                                                 esc_attr( $prefix_class ), esc_attr( $src ),
                                                esc_url( apply_filters( 'woocommerce_photo_reviews_grid_thumbnail_main', $src, $first_ele ) ),
                                                esc_attr( $image_alt ), esc_attr( $image_title ), esc_attr( $img_width ), esc_attr( $img_height )
                                        );
                                    }
                                ...
                                if ( ! in_array( 'image/' . strtolower( $file_type ), $settings->get_params( 'upload_allow_images' ) ) ) {
                                    if ( strpos( $first_ele, '.mp4' ) || strpos( $first_ele, '.webm' ) || strpos( $first_ele, '.mov' ) ) {
                                        printf( '<video class="%sreviews-images reviews-videos" src="%s" controls %s>%s</video>',
                                                 esc_attr( $prefix_class ), esc_url( $first_ele ), esc_attr( $auto_play_video ? 'autoplay muted' : '' ), esc_attr( $product_title )
                                        );
                                    } else {
                                        ...
                                        printf( '<iframe class="%s" src="%s" frameborder="0" allowfullscreen></iframe>',
                                                esc_attr( $video_class ), esc_url( $first_ele )
                                        );
                                    }
                                } else {
                                    printf( '<img class="%sreviews-images" loading="lazy" src="%s" alt="%s">',
                                             esc_attr( $prefix_class ), esc_url( $first_ele ), esc_attr( $product_title )
                                    );
                                }
                            }
                            if ( count( $img_post_ids ) > 1 ) {
