...
if ( isset( $cols ) ) {
	$grid_class = array(
		$prefix_class . 'wcpr-grid-layout-2-style',
		$prefix_class . 'wcpr-grid',
		$prefix_class . 'wcpr-grid-' . $cols . '-col',
...
foreach ( $my_comments as $v ) {
	...
	if ( $product ) {
		...
		?>
        <div id="<?php echo esc_attr( $prefix_class ); ?>comment-<?php echo esc_attr( $v->comment_ID ); ?>" class="<?php echo esc_attr( $prefix_class ); ?>wcpr-grid-layout-2-style-item">
            <div class="<?php echo esc_attr( $prefix_class ); ?>wcpr-content">
                ...
                <div class="<?php echo esc_attr( $prefix_class ); ?>review-author-container">
					...
					if ( '0' === $v->comment_approved ) {
                        ...
                        <div class="<?php echo esc_attr( $prefix_class ); ?>review-author-container-bottom">
                            <div class="review-author-container-bottom-left">
								<?php
								$get_comment_author_email = get_comment_author_email( $comment );
								echo get_avatar( $get_comment_author_email );
								?>
                            </div>
                            <div class="review-author-container-bottom-right">
								<?php
								if ( $is_wpml_active ) {
									?>
                                ...
                                <div class="wcpr-comment-author-rating">
                                    ...
                                    <div class="wcpr-comment-child">
										...
										if ( $comment_content_length > $max_content_length ) {
											...
											$comment_content   = sprintf( '<div class="%swcpr-review-content-short">%s<span class="%swcpr-read-more" title="%s">%s</span></div><div class="%swcpr-review-content-full">%s</div>',
												...
												esc_html__( '...More', 'woocommerce-photo-reviews' ), esc_attr( $prefix_class ), apply_filters( 'woocommerce_photo_reviews_grid_review_content', nl2br( $comment_content ), $comment ) );
										}
										?>
                ...
                <div class="<?php echo esc_attr( $prefix_class ); ?>review-content-container-bottom">
                    <div class="<?php echo esc_attr( $prefix_class ); ?>review-content-container">
                        ...
                        <div class="<?php echo esc_attr( $prefix_class ); ?>wcpr-wrap-review-content">
							...
							?>
                            <div class="<?php echo esc_attr( $prefix_class ); ?>wcpr-review-content"><?php echo wp_kses_post( $comment_content ); ?></div>
							<?php
							if ( $is_wpml_active ) {
								?>
                                <div>
                    ...
                    <div class="<?php echo esc_attr( $prefix_class ); ?>review-content-image">
						...
						if ( is_array( $img_post_ids ) && ! empty( $img_post_ids ) ) {
                            ...
                            foreach ( $img_post_ids as $img_post_id ) {
                                ...
                                $tmp_img_post_ids[] = $img_post_id;
                            }
                            $img_post_ids = array_unique( $tmp_img_post_ids );
                        }
						if ( is_array( $img_post_ids ) && ! empty( $img_post_ids ) ) {
							?>
                            <div class="<?php echo esc_attr( $prefix_class ); ?>reviews-images-container">
                                <div class="<?php echo esc_attr( $prefix_class ); ?>reviews-images-wrap-left">
									<?php
                                    $show_more_img = count($img_post_ids) - 4;
                                    $has_show_more_img = false;
									foreach ( $img_post_ids as $img_post_ids_k => $img_post_id ) {
										...
										if ( ! villatheme_is_url( $img_post_id ) ) {
                                            ...
                                            if (!$has_show_more_img && $show_more_img && $img_post_ids_k > 2){
                                                $has_show_more_img = true;
                                                $show_more = '<div class="'.$prefix_class.'images-qty">+'.$show_more_img.'</div>';
                                            }
											...
											if ( $is_video ) {
												printf( '<div class="%sreviews-images-wrap">%s<a data-image_index="%s" data-image_src="%s" data-image_caption="%s" href="%s"><video class="%sreviews-images reviews-videos" %s  >%s</video></a></div>',
													esc_attr( $prefix_class ),wp_kses_post($show_more), esc_attr( $img_post_ids_k ), esc_attr( $data_image_src ), esc_attr( $data_image_caption ),
													esc_url( apply_filters( 'woocommerce_photo_reviews_grid_thumbnail_main', $href, $img_post_id ) ),
													esc_attr( $prefix_class ),($img_post_ids_k < 4 || $show_more)? 'src="'.esc_url( $href ).'"': 'data-src="'.esc_url( $href ).'"',  esc_attr( $image_alt )
												);
											} else {
												printf( '<div class="%sreviews-images-wrap">%s<a data-image_index="%s" data-image_src="%s" data-image_caption="%s" href="%s"><img class="%sreviews-images" loading="lazy" %s alt="%s"></a></div>',
													esc_attr( $prefix_class ),wp_kses_post($show_more), esc_attr( $img_post_ids_k ), esc_attr( $href ), esc_attr( $data_image_caption ),
													esc_url( apply_filters( 'woocommerce_photo_reviews_grid_thumbnail_main', $href, $img_post_id ) ),
													esc_attr( $prefix_class ), ($img_post_ids_k < 4 || $show_more)? 'src="'.esc_url( $thumb ).'"': 'data-src="'.esc_url( $thumb ).'"', esc_attr( $image_alt )
												);
											}
										} else {
                                            if (!$has_show_more_img && $show_more_img && $img_post_ids_k > 2){
											...
											if ( ! in_array( 'image/' . strtolower( $file_type ), $settings->get_params( 'upload_allow_images' ) ) ) {
												if ( strpos( $img_post_id, '.mp4' ) || strpos( $img_post_id, '.webm' ) || strpos( $img_post_id, '.mov' ) ) {
													printf( '<div class="%sreviews-images-wrap">%s<a data-image_index="%s" href="%s"><video class="%sreviews-images reviews-videos" %s >%s</video></a></div>',
														esc_attr( $prefix_class ),wp_kses_post($show_more), esc_attr( $img_post_ids_k ), esc_attr( $img_post_id ),
														esc_attr( $prefix_class ),
                                                            ($img_post_ids_k < 4 || $show_more)? 'src="'.esc_url( $img_post_id ).'"': 'data-src="'.esc_url( $img_post_id ).'"',
                                                            esc_attr( $product_title )
													);
												} elseif ( strpos( $img_post_id, '.shopee.' ) ) {
													printf( '<div class="%sreviews-images-wrap">%s<a data-image_index="%s" href="%s"><img class="%sreviews-images" loading="lazy" %s alt="%s"></a></div>',
														esc_attr( $prefix_class ),wp_kses_post($show_more), esc_attr( $img_post_ids_k ), esc_attr( $img_post_id ),
														esc_attr( $prefix_class ),
                                                            ($img_post_ids_k < 4 || $show_more)? 'src="'.esc_url( $img_post_id ).'"': 'data-src="'.esc_url( $img_post_id ).'"',
                                                            esc_attr( $product_title )
													);
												} elseif ( strpos( $img_post_id, '.youtube.' ) ) {
													printf( '<div class="%sreviews-images-wrap">%s<a data-image_index="%s" href="%s"><iframe class="%sreviews-images reviews-videos reviews-videos-youtube" frameborder="0" allowfullscreen %s ></iframe></a></div>',
														esc_attr( $prefix_class ),wp_kses_post($show_more), esc_attr( $img_post_ids_k ), esc_attr( $img_post_id ),
														esc_attr( $prefix_class ),
                                                            ($img_post_ids_k < 4 || $show_more)? 'src="'.esc_url( $img_post_id ).'"': 'data-src="'.esc_url( $img_post_id ).'"'
													);
												} else {
													printf( '<div class="%sreviews-images-wrap">%s<a data-image_index="%s" href="%s"><video class="%sreviews-images reviews-iframe" %s >%s</video></a></div>',
														esc_attr( $prefix_class ),wp_kses_post($show_more), esc_attr( $img_post_ids_k ), esc_attr( $img_post_id ),
														esc_attr( $prefix_class ),
                                                            ($img_post_ids_k < 4 || $show_more)? 'src="'.esc_url( $img_post_id ).'"': 'data-src="'.esc_url( $img_post_id ).'"',
                                                            esc_attr( $product_title )
