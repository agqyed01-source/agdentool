...
foreach ( $my_comments as $v ) {
	...
	if ( $product ) {
        ...
        $items_class = [$prefix_class.'wcpr-grid-style-item'];
        if (is_array( $img_post_ids ) && ! empty( $img_post_ids )){
            $items_class[]= 'wcpr-grid-style-item-image';
        }
        if (empty($comment->comment_content)){
            $items_class[]='wcpr-grid-style-item-emty';
        }
		?>
        <div id="<?php echo esc_attr( $prefix_class ); ?>comment-<?php echo esc_attr( $v->comment_ID ); ?>"
             class="<?php echo esc_attr(implode(' ', $items_class) ) ?>">
