...
class WCPR_Elementor_Reviews_Widget extends Elementor\Widget_Base {
    ...
    protected function register_controls() {
        ...
        $this->add_control(
            'pagination_end',
            [
	            'label' => esc_html__('Only show pagination at the bottom of the reviews', 'woocommerce-photo-reviews'),
	            'type' => \Elementor\Controls_Manager::SWITCHER,
	            'default' => '',
	            'label_on' => esc_html__('Yes', 'woocommerce-photo-reviews'),
	            'label_off' => esc_html__('No', 'woocommerce-photo-reviews'),
        ...
        $this->add_control(
            ...
            [
                'label' => esc_html__('Hide if empty', 'woocommerce-photo-reviews'),
                'type' => \Elementor\Controls_Manager::SWITCHER,
                'default' => 'on',
                'label_on' => esc_html__('Yes', 'woocommerce-photo-reviews'),
                'label_off' => esc_html__('No', 'woocommerce-photo-reviews'),
		...
		$style =  $reviews_settings->get_params('photo', 'display');
        $this->add_control(
            'style',
            [
                'label' => esc_html__('Style', 'woocommerce-photo-reviews'),
                'type' => \Elementor\Controls_Manager::SELECT,
                'default' => $list_option_frontend_style[$style] ?? 'masonry',
                'options' => [
                    'masonry' => esc_html__('Masonry', 'woocommerce-photo-reviews'),
                    'normal' => esc_html__('Normal', 'woocommerce-photo-reviews'),
                    'grid' => esc_html__('Grid 1', 'woocommerce-photo-reviews'),
                    'grid_layout_2' => esc_html__('Grid 2', 'woocommerce-photo-reviews'),
                    'list_1' => esc_html__('List 1', 'woocommerce-photo-reviews'),
                    'list_2' => esc_html__('List 2', 'woocommerce-photo-reviews'),
                ],
            ]
        );
        $this->add_control(
        ...
        $this->add_responsive_control(
            'cols',
            [
                'label' => esc_html__('cols', 'woocommerce-photo-reviews'),
                'type' => \Elementor\Controls_Manager::NUMBER,
                'min' => 1,
                'max' => 5,
                'step' => 1,
                ...
                'conditions' => [
                    ...
                    'terms' => [
                        [
                            'name' => 'style',
                            'operator' => 'in',
                            'value' => ['masonry','grid','grid_layout_2'],
                        ],
                        [
                            'name' => 'is_slide',
                            'operator' => '===',
            ...
            [
                ...
                'selectors' => [
                    "{{WRAPPER}} .shortcode-wcpr-stars-count .star-rating:before,
					 ...
					 {{WRAPPER}} .shortcode-review-author-container .wcpr-comment-author-rating .star-rating span:before,
    ...
    public function get_shortcode_text() {
	    ...
	    if (is_array($settings) && !empty($settings)) {
			$exclude =['cols','cols_desktop','filter_enable'];
		    $shortcode_attr=["filter='".($settings['filter_enable'] ?? 'on')."'","cols='".($settings['cols'] ?? $settings['cols_desktop'] ?? 3)."'","is_elementor='yes'"];
		    foreach ($settings as $k => $v){
				if (in_array($k, ['products','products_status','product_cat','orderby','ratings'])){
	    ...
	    $shortcode = "[wc_photo_reviews_shortcode{$shortcode_attr}]";
	    return $shortcode;
    }
