jQuery(document).ready(function ($) {
    ...
    function triggerReviewClick() {
        $(document).on('click', '.wcpr-grid-item,.wcpr-grid-style-item,.wcpr-grid-layout-2-style-item,.wcpr-list-style-item', function () {
            let i;
        ...
        $(document).on('click','.reviews-images-wrap > a', function () {
            let this_image = $(this),
    ...
    function showReview(n) {
        ...
        if ($wrap_current.find('.reviews-images-container').length === 0) {
            ...
            $left_modal.find('.reviews-images').closest('a').on('click', function () {
                ...
                if (is_desktop) {
                    $left_main.find('.reviews-images').one('load', function () {
                        if (($('#wcpr-modal-wrap').outerHeight() - $left_main.outerHeight()) > 100) {
                            $('#reviews-content-left').css({position: 'unset'});
                            $left_modal.css({position: 'absolute', bottom: 0, left: 0, width: '500px'});
                        } else {
                            $('#reviews-content-left').css({position: 'relative'});
        ...
        let $right_meta = $('#reviews-content-right .reviews-content-right-meta');
        $right_meta.html($wrap_current.find('.review-content-container').html());
        if (!$right_meta.find('.wcpr-comment-author').length && $wrap_current.find('.review-author-container').length){
            $right_meta.prepend('<div class="review-author-container">'+$wrap_current.find('.review-author-container').html()+'</div>') ;
            $right_meta.find('.wcpr-review-rating').remove();
            $right_meta.find('.review-author-container .wcpr-review-date')
                .replaceWith('<div class="wcpr-comment-author-rating"><div class="wcpr-review-rating">'+$wrap_current.find('.wcpr-review-rating').html()+'</div><div class="wcpr-review-date">'+$wrap_current.find('.wcpr-review-date').html()+'</div></div>');
        }
        if (!$right_meta.find(' > .wcpr-wrap-review-helpfull').length && $right_meta.find('.wcpr-wrap-review-helpfull').length ){
            let $tmp = $right_meta.find('.wcpr-wrap-review-helpfull').clone();
            $right_meta.find('.wcpr-wrap-review-helpfull').remove();
            $right_meta.append($tmp);
        }
        $('.wcpr-modal-light-box').fadeIn(200);
        if (is_desktop) {
            if ($left_modal.find('.reviews-images').length) {
                $left_main.find('.reviews-images').one('load', function () {
                    if (($('#wcpr-modal-wrap').outerHeight() - $left_main.outerHeight()) > 100) {
                        $('#reviews-content-left').css({position: 'unset'});
                        $left_modal.css({position: 'absolute', bottom: 0, left: 0, width: '500px'});
    ...
    switch (woocommerce_photo_reviews_params.masonry_popup) {
        case 'review':
            ...
            break;
        case 'image':
            $(document).on('click', '.wcpr-grid-item .reviews-images, .review-content-image .reviews-images, .review-content-image .images-qty', function (e) {
                e.preventDefault();
                e.stopPropagation();
                let this_image = $(this);
