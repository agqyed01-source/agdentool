...
jQuery(document).ready(function ($) {
    ...
    $(document).on('click', '.viwcpr-review-order-submit:not(.viwcpr-review-order-submit-loading)', function () {
        let queue = [],
            button = $(this),
            form_container = $(this).closest('.viwcpr-review-order-wrap').find('.woocommerce-photo-reviews-form-container');
        if (!form_container.length) {
            return false;
        }
        let form = form_container.find('form');
        if (form.length && (!form.attr('enctype') || form.attr('enctype') !== 'multipart/form-data')) {
